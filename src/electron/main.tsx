...
import equalityEnvironment from '../util/equalityEnvironment';
import equalityConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!equalityEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    equalityConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!equalityConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !equalityConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (equalityConfig.manageDaemonLifetime()) {
        equalityEnvironment.startEqualityDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Equality Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Equality-Network/equality-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Equality-Network/equality-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Equality-Network/equality-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Equality-Network/equality-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Equality-Network/equality-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://discord.gg/TgJyxs1234');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/equality_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Equality Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Equality' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Equality Blockchain' }),
            click: () => {
              openAbout();
            },
          },
