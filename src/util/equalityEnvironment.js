...
const PY_FOLDER = '../equality/daemon';
...
const getEqualityVersion = () => {
  let version = null;
  const exePath = getExecutablePath('equality');
  // first see if we can get a equality exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or equality is on the path
    try {
      version = child_process
