...
global.daemon_rpc_ws = `wss://${self_hostname}:55421`;
...
function loadConfig(net) {
  try {
    // check if EQUALITY_ROOT is set. it overrides 'net'
    const config_root_dir =
      'EQUALITY_ROOT' in process.env
        ? process.env.EQUALITY_ROOT
        : path.join(os.homedir(), '.equality', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 55421); // jshint ignore:line
