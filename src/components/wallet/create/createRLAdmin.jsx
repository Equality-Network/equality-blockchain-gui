...
import { AlertDialog } from '@equality/core';
import {
  Typography,
  Button,
  Box,
...
import { equality_to_mojo } from '../../../util/equality';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      equalityper_input.value === '' ||
      Number(equalityper_input.value) === 0 ||
      !Number(equalityper_input.value) ||
      isNaN(Number(equalityper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const equalityper = equality_to_mojo(equalityper_input.value);
    const equalityper_value = Number.parseInt(Number(equalityper));
    ...
    // var fee = equality_to_mojo(fee_input.value);
