...
import { AlertDialog, Card, Flex } from '@equality/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const equalityper_input = ip_parsed.limit;
    ...
    const equalityper_value = Number.parseInt(Number(equalityper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (equality per interval):{' '}
                  {mojo_to_equality_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
