...
import { Card, FormatLargeNumber, Link, Loading, TooltipIcon, Flex } from '@equality/core';
import {
  unix_to_short_date,
  hex_to_array,
  arr_to_hex,
...
export default function Block() {
  ...
  const poolReward = mojo_to_equality(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_equality(calculateBaseFarmerReward(blockRecord.height));

  const equalityFees = blockRecord.fees
    ? mojo_to_equality(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: equalityFees ? `${equalityFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    >
      <Card
        title={(
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Equality
              blockchain
            </Trans>
          </BlockTitle>
        )}
